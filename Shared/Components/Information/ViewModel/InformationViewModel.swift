...
class InformationViewModel {
    ...
    init(date: Date) {
        let firstSection = "Day \(date.getDayNumber(date: date))"
        titles.append(firstSection)
    ...
    func infoFilter(type: DataTrackingType) -> [InfoData] {
        return infos.filter {$0.type == type}
    }
